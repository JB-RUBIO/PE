...
def exponentialLawDelay(dicToPrint, lamb=2):
    return expon.rvs(1/lamb, size=len(dicToPrint))


def getOrders(dicToPrint, campus, date):
    path = r'res\formatter\forecast.xlsm'

    campusDataFrame = dg.readCampus(path)
    forecastDataFrame = dg.readForecast(path)

    dfOrders = dg.createDateDataFrame(forecastDataFrame)
    dfOrders = dg.preprocessing(dfOrders, forecastDataFrame, campusDataFrame)

    dfCampus = dg.campusCompletion(path, campusDataFrame)
    dfProducer = dg.createProducerDataFrame(dfCampus)

    dicOrders = dg.uniformDistribution(dfOrders, dfProducer)

    res = []
    for deliverers in dicToPrint:
        for producer in dicToPrint[deliverers]:
            buffer = 0
            if type(dicToPrint[deliverers]) == rh.Prod:
                print('something')
            else:
                print('something')
...
def solveWithSolver(Arrival_Time, epsilon, NB_Clients, Personnes_Distribution, Tps_Distribution, Lmax):
    ...
    for i in indice:
        prob += (dvar_arrivee_clients[i] +
                 Tps_Distribution <= dvar_depart_clients[i]),
        "condition_arrivee_distribution_{0}".format(i)
        prob += (dvar_depart_clients[i] <= Lmax),
        "Lapse_temps_distribution{0}".format(i)
